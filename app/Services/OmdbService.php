...
class OmdbService
{
    ...
    public function findInOmdb($data)
    {
        ...
        $movie = $GuzzleClient->request('GET', 'http://www.omdbapi.com/', [
            'query' => [
                'apikey' => self::API_KEY,
                'type' => 'movie',
                't' => $title,
...
//            $actorsNamesArray = explode(', ', $actorsNames);
    ...
    private function createActors(array $actorsNames)
    {
        $actors = [];
        foreach ($actorsNames as $actorName) {
            $actor = Actor::where('full_name', $actorName)->first();
            if (!$actor) {
                $actor = $this->createActor($actorName);
            }
            array_push($actorIds, $actor->id);
        }
     ...
     */
    private function createActor(string $name)
    {
        return $this->personService->createActor(['full_name' => $name, 'movie_ids' => [1]]);
    }
